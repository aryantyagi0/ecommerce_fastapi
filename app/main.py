...
@app.delete("/users/", response_model=schemas.User, summary="Delete My Account", description="Deletes the account of the currently logged-in user.", tags=["Users"])
...
def login(form_data: OAuth2PasswordRequestForm = Depends(), db: Session = Depends(get_db)):
    ...
    token = create_access_token(data={"user_id": user.id, "email": user.email, "role": user.role}, expires_delta=timedelta(minutes=60))
...
@app.put("/addresses/update", response_model=schemas.Address, tags=["Addresses"])
...
def create_product(
    ...
    current_user: models.User = Depends(get_current_admin_or_seller),  # 👈 restrict access
):
...
@app.get("/seller/products/", response_model=List[schemas.Product], tags=["Products"])
...
@app.put("/seller/products/{product_id}", response_model=schemas.Product, tags=["Products"])
...
@app.delete("/seller/products/{product_id}", tags=["Products"])
...
@app.delete("/admin/products/{product_id}", tags=["Products"])
...
# ---------- CART ----------
# @app.get("/cart/", response_model=schemas.Cart, tags=["Cart"])
...
# @app.post("/cart/items", response_model=schemas.CartItem, tags=["Cart"])
...
# @app.put("/cart/items/{cart_item_id}", response_model=schemas.CartItem, tags=["Cart"])
...
# @app.delete("/cart/items/{cart_item_id}", tags=["Cart"])
...
# def create_order_from_cart(db: Session = Depends(get_db), current_user: models.User = Depends(get_current_user)):
...
# @app.get("/orders/detail/{order_id}", response_model=schemas.Order, tags=["Orders"])
...
# ---------- WISHLIST ----------
@app.post("/wishlist/", response_model=schemas.WishlistResponse, status_code=201, tags=["Wishlist"])
...
@app.get("/wishlist/", response_model=List[schemas.WishlistResponse], tags=["Wishlist"])
...
@app.delete("/wishlist/{wishlist_id}", status_code=204, tags=["Wishlist"])
...
# ---------- REVIEWS ----------
@app.post("/reviews/", response_model=schemas.ReviewResponse, status_code=201, tags=["Reviews"])
...
@app.get("/reviews/{product_id}", response_model=List[schemas.ReviewResponse], tags=["Reviews"])
def get_reviews(product_id: int, db: Session = Depends(get_db)):
    return crud.get_reviews_for_product(db, product_id=product_id)

# ---------- SHIPMENTS ----------
@app.post("/shipments/", response_model=schemas.ShipmentResponse, status_code=201, tags=["Shipments"])
...
@app.get("/shipments/", response_model=List[schemas.ShipmentResponse], tags=["Shipments"])
...
@app.get("/shipments/{shipment_id}", response_model=schemas.ShipmentResponse, tags=["Shipments"])
...
@app.put("/shipments/{shipment_id}", response_model=schemas.ShipmentResponse, tags=["Shipments"])
